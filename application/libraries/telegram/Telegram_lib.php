...
class Telegram_lib
{
    ...
    public function sendmsg($msg, $chat_id, $parse_mode)
    {
        $telegram = new Telegram($this->ci->config->item('bot_token', 'telegram'));
        $content = array('chat_id' => $chat_id, 'text' => $msg, 'parse_mode' => $parse_mode);
        return $telegram->sendMessage($content);
    }
    public function sendlocation($lat, $long, $chat_id)
    {
        $telegram = new Telegram($this->ci->config->item('bot_token', 'telegram'));
        $content = ['chat_id' => $chat_id, 'latitude' => $lat, 'longitude' => $long];
        return $telegram->sendLocation($content);
    }
    public function sendimg($img_path, $caption = false, $chat_id)
    {
        $telegram = new Telegram($this->ci->config->item('bot_token', 'telegram'));
